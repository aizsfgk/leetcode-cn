...
public:
    int maxConsecutiveAnswers(string answerKey, int k) {
        // 遍历2次，求最大值
        return max(canMaxAlterCharNum(answerKey, k, 'T'), canMaxAlterCharNum(answerKey, k, 'F'));
    }

    int canMaxAlterCharNum(const string s, int k, char c) {
        int n = s.size();
        int cnt = 0, ans = 0;
        for (int l=0, r=0; r<n; r++) {
            if (s[r] == c) cnt++;

            while (cnt > k) { // 超过修改的上限，需要缩短左边界
                if (s[l] == c) cnt--;
                l++;
            }

            ans = max(ans, r-l+1); // 修改后的最长字符串长度
        }
        return ans;
    }
};
