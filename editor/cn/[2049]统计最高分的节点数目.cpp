...
public:
    int countHighestScoreNodes(vector<int>& parents) {
        ...
        dfs(0, left, right); // 深度遍历
        return ans;
    }

    int dfs(int node, const vector<int> &left, const vector<int> &right) {
        // 值为-1；不合法，返回0
        if (node == -1)
            return 0;

        // 左边节点个数
        int leftCnt = dfs(left[node], left, right);
        // 右边节点个数
        int rightCnt = dfs(right[node], left, right);
        int remain = n - leftCnt - rightCnt - 1;

        // 后续遍历；获取结果后，操作
        long long score = helper(leftCnt) * helper(rightCnt) * helper(remain);
        if (score == maxScore) {
            ans++;
        ...
        // 返回数据
        return leftCnt + rightCnt + 1; // 只有根节点，所以+1
    }
