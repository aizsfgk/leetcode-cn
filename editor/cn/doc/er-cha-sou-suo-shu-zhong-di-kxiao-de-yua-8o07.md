...
- 时间复杂度：*O(H+k)*，其中 *H* 是树的高度。在开始遍历之前，我们需要 *O(H)* 到达叶结点。当树是平衡树时，时间复杂度取得最小值 ![O(\logN+k) ](./p__O_log_N_+_k__.png) ；当树是线性树（树中每个结点都只有一个子结点或没有子结点）时，时间复杂度取得最大值 *O(N+k)*。

- 空间复杂度：*O(H)*，栈中最多需要存储 *H* 个元素。当树是平衡树时，空间复杂度取得最小值 ![O(\logN) ](./p__O_log_N__.png) ；当树是线性树时，空间复杂度取得最大值 *O(N)*。
...
- 令 ![\textit{node} ](./p__textit{node}_.png)  等于根结点，开始搜索。

- 对当前结点 ![\textit{node} ](./p__textit{node}_.png)  进行如下操作：
  - 如果 ![\textit{node} ](./p__textit{node}_.png)  的左子树的结点数 ![\textit{left} ](./p__textit{left}_.png)  小于 *k-1*，则第 *k* 小的元素一定在 ![\textit{node} ](./p__textit{node}_.png)  的右子树中，令 ![\textit{node} ](./p__textit{node}_.png)  等于其的右子结点，*k* 等于 ![k-\textit{left}-1 ](./p__k_-_textit{left}_-_1_.png) ，并继续搜索；
  - 如果 ![\textit{node} ](./p__textit{node}_.png)  的左子树的结点数 ![\textit{left} ](./p__textit{left}_.png)  等于 *k-1*，则第 *k* 小的元素即为 *node* ，结束搜索并返回 ![\textit{node} ](./p__textit{node}_.png)  即可；
  - 如果 ![\textit{node} ](./p__textit{node}_.png)  的左子树的结点数 ![\textit{left} ](./p__textit{left}_.png)  大于 *k-1*，则第 *k* 小的元素一定在 ![\textit{node} ](./p__textit{node}_.png)  的左子树中，令 ![\textit{node} ](./p__textit{node}_.png)  等于其左子结点，并继续搜索。
...
- 时间复杂度：预处理的时间复杂度为 *O(N)*，其中 *N* 是树中结点的总数；我们需要遍历树中所有结点来统计以每个结点为根结点的子树的结点数。搜索的时间复杂度为 *O(H)*，其中 *H* 是树的高度；当树是平衡树时，时间复杂度取得最小值 ![O(\logN) ](./p__O_log_N__.png) ；当树是线性树时，时间复杂度取得最大值 *O(N)*。
...
- 一棵存有 *n* 个结点的平衡二叉搜索树的高度是 ![O(\logn) ](./p__O_log_n__.png) 。

**思路和算法**

我们注意到在方法二中搜索二叉搜索树的时间复杂度为 *O(H)*，其中 *H* 是树的高度；当树是平衡树时，时间复杂度取得最小值 ![O(\logN) ](./p__O_log_N__.png) 。因此，我们在记录子树的结点数的基础上，将二叉搜索树转换为平衡二叉搜索树，并在插入和删除操作中维护它的平衡状态。
...
- 时间复杂度：预处理的时间复杂度为 *O(N)*，其中 *N* 是树中结点的总数。插入、删除和搜索的时间复杂度均为 ![O(\logN) ](./p__O_log_N__.png) 。
